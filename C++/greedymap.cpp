// clang++ greedymap.cpp -O3 -o greedy
...
{
public:
    std::map<string, Activity *> activity; 
...
void importCSV(std::list<string>& lines, std::map<string, Activity *> *al, std::list<Resource *> *rl)
{
    string delimiter = ",";
...
void scheduleResources(SchemaData& sd) {
    for (std::list<Resource *>::iterator it = sd.resource.begin(); it != sd.resource.end(); ++it)
    {
        for (int c = 0; c < 50; c++) 
        {
            ...
            std::map<string, Activity *>::iterator lowestobj;

            // See which is closest
            for (std::map<string, Activity *>::iterator ait = sd.activity.begin(); ait != sd.activity.end(); ++ait)
            {
                double dist = distanceBetweenPointsLatLong((*it)->lat, (*it)->lon, ait->second->lat, ait->second->lon);
                if (dist < lowest) 
                {
                    lowest = dist;
                    lowestobj = ait;
