...
fn build(lines: Vec<&str>) -> SchemaData {
    ...
    for x in lines.iter() {
        ...
        if split.len() == 3 {
            ...
            sd.activity
                .insert(id.to_string(), Activity::new(&String::from(id), lat, lng));
        }
    }
    sd
}
...
fn distance_between_points(lat1: f64, lon1: f64, lat2: f64, lon2: f64) -> f64 {
    ...
    let a: f64 = sin_half_lat * sin_half_lat
        + (start_lat_in_rad).cos() * (end_lat_in_rad).cos() * sin_half_lon * sin_half_lon;
    let c: f64 = a.sqrt().atan2((1.0 - a).sqrt());
    EARTH_RADIUS_M * (c + c)
}

fn schedule_ind(act: &Vec<Activity>, id: &String, lat: f64, lon: f64) -> (Allocation, usize) {
    let out: (usize, f64) = act
        .into_iter()
        .enumerate()
        .map(|(j, x)| (j, distance_between_points(lat, lon, x.lat, x.lon)))
        .fold(
            (0, f64::INFINITY),
            |acc, x| if x.1 < acc.1 { x } else { acc },
        );
    let a = Allocation::new(id, &act[out.0].id, out.1);
    (a, out.0 as usize)
}

fn schedule_all<'a>(act: &'a HashMap<String, Activity>, lat: f64, lon: f64) -> Vec<(String, f64)> {
    let mut out: Vec<(String, f64)> = act
        .values()
        .map(|x| (x.id.clone(), distance_between_points(lat, lon, x.lat, x.lon)))
        .collect();
...
fn schedule_resources(sd: &mut SchemaData) -> f64 {
    ...
    for res in sd.resource.iter() {
        let results = schedule_all(&sd.activity, res.lat, res.lon);
        for (activity, distance) in results {
            let a = Allocation::new(&res.id, &activity, distance);
            allocation.push(a);

            // Now remove
            sd.activity.remove(&activity);
        }
    }
